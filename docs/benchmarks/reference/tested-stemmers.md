...
| --- | --- | --- | --- | --- | --- |
| Radixor | Egothor / Radixor project | Base commit and measured working-tree state recorded on the environment page | All 143 default languages have quality and benchmark pages; speed covers 144 user-facing models including optional PoliMorf | Deterministic preferred patch via `get`; ranked distinct alternatives via `getAll`; primary is always included | Dictionary-trained compiled patch trie. Default rows use each language's stable default model ID. |
...
| Official Snowball Java | Snowball project | 3.1.0 source distribution; SHA-256 `5dab34d491f55f47b6e971569ffe6aadf5991512c648ddfe5d331b494cf6d655` | 29 active exact-language direct adapters; all 29 participate in the homepage aggregate | Direct generated Java API; single output | Rule-based suffix algorithms provide broad baselines rather than dictionary-root guarantees. Lucene 10.5.0 exposes 25 of these active algorithms through `SnowballFilter`; Czech, Persian, Polish, and Southern Sotho use the direct API only. |
