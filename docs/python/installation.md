...
`radixor-models-standard>=3.0,<4.0`; an installation of `radixor`
...
- **Model packaging.** Neither runtime distribution contains textual
  dictionaries. `radixor-models-standard` ships 31 compiled gzip v7 resources,
...
- **Catalog compatibility.** The current supported Python runtime accepts the
  documented compatible model-distribution major (`>=3.0,<4.0`) carrying an
